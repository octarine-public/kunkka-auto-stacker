import { Ability, ArrayExtensions, Color, Creep, EntityManager, EventsSDK, GameRules, GameState, GUIInfo, Hero, kunkka_torrent, Menu, NeutralSpawnBox, RendererSDK, TickSleeper, Unit, Vector2, Vector3, WardObserver } from "github.com/octarine-public/wrapper/wrapper/Imports"
...
function FindEligibleCamps(
	...
	const my_vec = owner.VisualPosition,
		cast_range = abil.CastRange
	return ArrayExtensions.orderBy(
		GameRules!.NeutralSpawnBoxes.filter(spot => (
			!EntityManager.GetEntitiesByClass(Hero).some(ent => spot.Includes(ent.VisualPosition))
			&& !EntityManager.GetEntitiesByClass(Creep).some(ent => !ent.IsNeutral && spot.Includes(ent.VisualPosition))
		...
		)).map(spot => [spot, EntityManager.GetEntitiesByClass(Creep).filter(x => (
			(!x.IsVisible || !x.IsWaitingToSpawn)
			&& x.IsAlive
			&& spot.Includes(x.VisualPosition)
		))] as [NeutralSpawnBox, Creep[]]).filter(([spot, creeps]) => (
			creeps.length !== 0
		...
		] as [NeutralSpawnBox, Creep[], Vector3]).filter(([spot, creeps, center]) => {
			...
			return center.IsInRange(my_vec, cast_range) && time > 0 && time < 0.15
		}),
		spot => spot[2].Distance2D(my_vec),
	)
}
EventsSDK.on("Tick", () => {
	if (!State.value || sleeper.Sleeping || GameRules === undefined || GameRules.IsPaused)
		return
	const cur_time = GameRules.GameTime
	if (cur_time < 60)
		return
	EntityManager.GetEntitiesByClass(kunkka_torrent).forEach(abil => {
		...
		if (owner === undefined || !owner.IsControllable || !abil.CanBeCasted())
			return
		const camp = FindEligibleCamps(abil, owner, cur_time)[0]
		if (camp !== undefined) {
			const center = camp[2]
...
EventsSDK.on("Draw", () => {
	...
	EntityManager.GetEntitiesByClass(kunkka_torrent).forEach(abil => {
		...
		const my_vec = owner.VisualPosition,
			cast_range = abil.CastRange
		FindEligibleCamps(abil, owner, 0, false).forEach(([spot, creeps, center]) => {
			if (!GlobalVisualsState.value && !center.IsInRange(my_vec, cast_range))
				return
			stacking ??= center
			camps.add(center)
		})
